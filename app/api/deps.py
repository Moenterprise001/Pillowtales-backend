...
def get_story_service(story_repo: StoryRepository = Depends(get_story_repo)) -> StoryService:
    return StoryService(story_repo)


def get_narration_service(story_repo: StoryRepository = Depends(get_story_repo), user_repo: UserRepository = Depends(get_user_repo), subscription_service: SubscriptionService = Depends(get_subscription_service)) -> NarrationService:
    return NarrationService(story_repo, user_repo, subscription_service)
