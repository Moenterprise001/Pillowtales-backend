...
class NarrationService:
    ...
    def get_page_status(self, user_id: str, story_id: str, narrator: Optional[str], lang: Optional[str]) -> PageStatusResponse:
        ...
        # IMPORTANT: page-status must remain passive.
        # It may report ready/generating/failed state, but it must not start,
        # restart, expand, or mutate narration jobs. Generation orchestration
        # belongs to request_narration() only. This prevents polling races that
        # can cause page skips, pauses, or stuck buttons.
