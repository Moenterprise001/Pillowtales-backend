...
class AuthService:
    ...
    def verify_token(self, token: str) -> dict:
        ...
        try:
            ...
            if response and response.user:
                return {'user_id': response.user.id, 'sub': response.user.id, 'email': response.user.email}
        except Exception:
            pass
    ...
    def login(self, request: LoginRequest, user_repo: UserRepository) -> AuthResponse:
        ...
        return AuthResponse(token=token, userId=auth_response.user.id, email=request.email, preferredLanguage=profile.get('preferred_language', 'en'))
