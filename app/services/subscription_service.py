...
class SubscriptionService:
    ...
    def get_subscription(self, user_id: str, user_email: Optional[str]) -> SubscriptionResponse:
        ...
        is_tester = bool(
            normalized_email and normalized_email in PREMIUM_TESTER_EMAILS
        )

        parent_voice_bypass = bool(
            normalized_email and normalized_email in QA_PARENT_VOICE_BYPASS_EMAILS
        )
        ...
        narration_count = self.story_repo.count_narrations_since(
            user_id,
            week_ago
        )
        ...
        weekly_limit = (
            None
            if is_premium
            else SUBSCRIPTION_TIERS['free']['weekly_narration_limit']
        )

        can_narrate = (
            is_premium
            or narration_count < (weekly_limit or 0)
        )

        remaining = (
            None
            if weekly_limit is None
            else max(0, weekly_limit - narration_count)
        )
    ...
    def get_tier_payload(self, subscription: SubscriptionResponse) -> dict:
        ...
        return {
            ...
            'available_narrators': [
                {'id': n, **VOICE_PRESETS[n]}
                for n in tier['narrators']
                if n in VOICE_PRESETS
            ],
            'available_companions': [
                {'id': c, **STORY_COMPANIONS[c]}
                for c in tier['companions']
                if c in STORY_COMPANIONS
            ],
            ...
            if item_id == 'parent_voice':
                return {
                    ...
                    'reason': None if (
                        ...
                        or subscription.parent_voice_credits > 0
                        or subscription.parent_voice_intro_available
                    ) else 'parent_voice_purchase_required',
