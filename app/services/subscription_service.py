    ...
    def feature_allowed(self, subscription: SubscriptionResponse, feature: str, item_id: Optional[str] = None) -> dict:
        ...
        if feature == 'narrator' and item_id and item_id not in tier['narrators']:
            if item_id == 'parent_voice':
                return {
                    'allowed': subscription.is_premium or subscription.parent_voice_credits > 0 or subscription.parent_voice_intro_available,
                    'upgrade_required': not (subscription.is_premium or subscription.parent_voice_credits > 0 or subscription.parent_voice_intro_available),
                    'reason': None if (subscription.is_premium or subscription.parent_voice_credits > 0 or subscription.parent_voice_intro_available) else 'parent_voice_purchase_required',
